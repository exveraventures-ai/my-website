...
// Test data generation script for WorkWell
...
// Generate random hours with realistic distribution (40-80 hours/week)
function generateWeeklyHours() {
  // Bell curve centered around 55-60 hours
  const mean = 57
  const stdDev = 12
  const hours = mean + (Math.random() + Math.random() + Math.random() + Math.random() - 2) * stdDev
  return Math.max(30, Math.min(85, hours)) // Clamp between 30-85
}

// Generate date for the past N days
...
function generateUser(index) {
  ...
  const email = `${firstName.toLowerCase()}.${lastName.toLowerCase()}.${index}@testworkwell.com`
  ...
  return {
    ...
    is_pro: Math.random() > 0.7, // 30% are pro users
    age: 24 + Math.floor(Math.random() * 20), // 24-44
    gender: Math.random() > 0.5 ? 'Male' : 'Female',
    height_cm: 160 + Math.floor(Math.random() * 30), // 160-190 cm
    weight_kg: 55 + Math.floor(Math.random() * 45), // 55-100 kg
    created_at: new Date().toISOString()
  }
}

// Generate work logs for a user
function generateWorkLogs(userId, numDays = 60) {
  const logs = []

  for (let i = 0; i < numDays; i++) {
    // 85% chance of logging each day (realistic - people miss days)
    if (Math.random() > 0.15) {
      const date = new Date()
      date.setDate(date.getDate() - i)

      const dailyHours = generateWeeklyHours() / 5 // Convert weekly to daily
...
async function generateTestData() {
  ...
  const NUM_USERS = 50 // Adjust this number as needed
  const LOGS_PER_USER = 60 // Days of logs
...
// Function to calculate and insert cohort statistics
async function generateCohortStats() {
  try {
    ...
    for (const [cohortKey, cohort] of Object.entries(cohorts)) {
      ...
      // Get all work logs for this cohort
      ...
      // Calculate average weekly hours per user
      const weeklyAverages = Object.values(userWeeklyHours).map(hours => {
        ...
        return (total / days) * 7 // Convert to weekly
      })
...
export { generateUser, generateWorkLogs, generateTestData, generateCohortStats }